...
def run_grain128(data):
    rng = Random()
    
    key = rng.randbytes(16)
    nonce = rng.randbytes(12)
    ad = bytes("grain128-test")
    
    start_time = time.time()
    encrypted, tag = grain_128aead.encrypt(key, nonce, ad, data)
    print_process_info()
    ...
    flg, dec = grain_128aead.decrypt(key, nonce, tag, ad, encrypted)
    assert flg
