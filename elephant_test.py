...
def run_elephant(data):
    rng = Random()
    
    key = rng.randbytes(16)
    nonce = rng.randbytes(12)
    ad = bytes("elephant-test")
    
    start_time = time.time()
    encrypted, tag = elephant.dumbo_encrypt(key, nonce, ad, data)
    print_process_info()
    ...
    flg, dec = elephant.dumbo_decrypt(key, nonce, tag, ad, encrypted)
    assert flg
