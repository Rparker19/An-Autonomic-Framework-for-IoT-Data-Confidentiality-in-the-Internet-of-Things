...
def run_tinyjambu(data):
    rng = Random()
    
    key = rng.randbytes(16)
    nonce = rng.randbytes(12)
    ad = bytes("tinyjambu-test")
    
    start_time = time.time()
    encrypted, tag = tinyjambu.tinyjambu_128_encrypt(key, nonce, ad, data)
    print_process_info()
    ...
    flg, dec = tinyjambu.tinyjambu_128_decrypt(key, nonce, tag, ad, encrypted)
    assert flg
